    ...
    public function getData()
    {
        return \Datatable::collection(Comment::All())
                        ...
                        ->addColumn('status', function ($model) {
                            $status = $model->status;
                            if ($status == 1) {
                                return '<p style="color:blue"">'.\trans('lang.published');
                            } else {
                                return '<p style="color:red"">'.\trans('lang.not_published');
                            }
                        })

                        ->addColumn('Actions', function ($model) {
                            return '<div class="row"><a href=comment/delete/'.$model->id.' class="btn btn-danger btn-xs">'.\trans('lang.delete').'</a>&nbsp;<a href=published/'.$model->id.' class="btn btn-warning btn-xs">'.\trans('lang.publish').'</a></div>';
                        })
                        ->make();
    }
